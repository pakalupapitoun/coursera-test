...
function calculate(){
  ...
  for(i = 0; i < arrival_time.length; i++)
  {
    turn_around_time[index_array[i]] = completion_time[index_array[i]] - 
                  arrival_time[index_array[i]];
    waiting_time[index_array[i]] = turn_around_time[index_array[i]] - 
                 burst_time[index_array[i]];
  }	
  
		//total turn around and waiting time
		for (i = 0; i < arrival_time.length ; i++) 
        { 
         WT_total = WT_total + waiting_time[index_array[i]]; 
         TAT_total = TAT_total + turn_around_time[[index_array[i]]]; 
        } 
